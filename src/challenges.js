...
function howManyTimes(param, wordToCount) {
  if (param.length === 0) {
    return 0;
  }
...
function multiplyBy(param, multiplier) {
  if (param.length === 0) {
    return [];
  }
  let newNumbers = [];
...
function filterOut(param, paramFilter) {
  if (!param.length) {
    return null;
  }
  ...
  for (let i = 0; i < param.length; i++) {
    if (!paramFilter.includes(param[i])) {
      //si param[i] no esta en paramFilter devuelvemelo
      //si lo que esta en param no coincide con lo que esta en paramFilter, devuelvemelo
